...
import { count, desc, asc, ilike, and, gte, or, eq } from 'drizzle-orm';
...
export async function getUserStats(): Promise<UserStats> {
	...
	try {
		...
		const activeUsersResult = await db.select({ count: count() }).from(user).where(eq(user.emailVerified, true));

		// Get new users (registered in the last 30 days)
		const thirtyDaysAgo = new Date();
		thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);

		const newUsersResult = await db.select({ count: count() }).from(user).where(gte(user.createdAt, thirtyDaysAgo));
