...
import { useAuthLoading, useIsAuthenticated, useAuthInitialized } from '@/store/auth-store';
			...
			<div className='flex min-h-screen items-center justify-center'>
				<Card className='w-full max-w-md'>
					<CardHeader className='text-center'>
						...
						<CardDescription>此页面仅限管理员访问。如果您认为这是错误，请联系系统管理员。</CardDescription>
					</CardHeader>
					<CardContent className='space-y-4'>
						<Button onClick={() => router.push(redirectTo)} className='w-full' variant='solid'>
							<ArrowLeft className='mr-2 h-4 w-4' />
							返回设置
						</Button>
						<Button onPress={() => router.push('/')} className='w-full' variant='bordered'>
							返回首页
						</Button>
					</CardContent>
				</Card>
