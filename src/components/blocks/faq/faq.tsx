	...
	return (
		<section className='py-16'>
			<div className='container space-y-16'>
				...
				<Accordion className='mx-auto w-full lg:max-w-3xl'>
					{items.map((item) => (
						<AccordionItem key={item.id} title={item.question} value={item.id}>
							{item.answer}
						</AccordionItem>
					))}
				</Accordion>
				<div className='mx-auto flex max-w-4xl flex-col items-center rounded-lg bg-accent p-4 text-center md:rounded-xl md:p-6 lg:p-8'>
					<div className='relative'>
						<Avatar
							showFallback
							src='/avatar/2.png'
							className='-translate-x-[60%] absolute mb-4 size-16 origin-bottom scale-[80%] border bg-white md:mb-5'
						/>
						<Avatar
							className='absolute mb-4 size-16 origin-bottom translate-x-[60%] scale-[80%] border bg-white md:mb-5'
							showFallback
							src='/avatar/4.png'
						/>
						<Avatar className='mb-4 size-16 border bg-white md:mb-5' src='/avatar/5.png' showFallback />
					</div>
					...
					<div className='flex w-full flex-col justify-center gap-2 sm:flex-row'>
						<Button className='w-full sm:w-auto' as={Link} href={supportButtonUrl} target='_blank' rel='noreferrer'>
							{supportButtonText}
						</Button>
					</div>
				</div>
