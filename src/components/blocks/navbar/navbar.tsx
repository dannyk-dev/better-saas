...
function DesktopAuthDisplay({
	...
	return (
		<div className='flex items-center gap-2'>
			<Button as={Link} href={loginUrl} variant='light' >
				{loginText}
			</Button>
			<Button as={Link} href={signupUrl} color='primary'>
				{signupText}
			</Button>
		</div>
	);
...
function MobileAuthDisplay({
	...
	return (
		<div className='flex flex-col gap-2'>
			<Button as={Link} href={loginUrl} variant='flat' size='lg' className='w-full justify-start'>
				{loginText}
			</Button>
			<Button as={Link} href={signupUrl} size='lg' className='w-full justify-start'>
				{signupText}
			</Button>
		</div>
	);
...
const Navbar = ({
								...
								<li key={subItem.title}>
									<NavigationMenuLink asChild>
										...
										>
											...
											<p className='line-clamp-2 text-muted-foreground text-sm leading-snug'>{subItem.description}</p>
										</Link>
									</NavigationMenuLink>
								</li>
							))}
	...
	const renderMobileMenuItem = (item: MenuItem) => {
		if (item.items && item.items.length > 0) {
			return (
				<AccordionItem key={item.title} title={item.title} className='border-b-0'>
						...
						{item.items.map((subItem) => (
							...
							>
								...
								<div>
									...
									{subItem.description && <div className='text-muted-foreground text-xs'>{subItem.description}</div>}
								</div>
							</Link>
						))}
					</div>
