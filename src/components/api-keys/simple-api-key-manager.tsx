				...
				{newlyCreatedKey && (
					<Card className='border-green-200 bg-green-50'>
						<CardHeader className='pb-3'>
							...
							<CardDescription className='text-green-600'>请立即复制并保存此密钥，它只会显示一次</CardDescription>
						</CardHeader>
						<CardContent className='pt-0'>
							<div className='flex items-center gap-2'>
								...
								/>
								<Button variant='bordered' size='sm' onPress={() => setShowNewKey(!showNewKey)}>
									{showNewKey ? <EyeOff className='h-4 w-4' /> : <Eye className='h-4 w-4' />}
								</Button>
								<Button variant='bordered' size='sm' onPress={() => copyToClipboard(newlyCreatedKey)}>
									<Copy className='h-4 w-4' />
								</Button>
							</div>
							<Button variant='bordered' size='sm' className='mt-2' onClick={() => setNewlyCreatedKey(null)}>
								我已保存，关闭
							</Button>
						</CardContent>
					</Card>
							...
							{(onClose) => (
								<>
										...
										<div className='space-y-4'>
											<div>
												...
												<Input
													...
													value={newKeyName}
													onValueChange={(e) => setNewKeyName(e)}
												/>
											</div>
											<div>
												...
												<Input
													...
													type='datetime-local'
													value={newKeyExpiry}
													onValueChange={(e) => setNewKeyExpiry(e)}
												/>
											</div>
										</div>
									</ModalBody>
									<ModalFooter>
										...
										<Button
											onPress={async () => {
												await createApiKey().then(() => onClose());
											}}
											disabled={creating}
										>
											{creating ? '创建中...' : '创建'}
								...
								<div className='flex items-center justify-between'>
									<div className='flex-1'>
										...
										<div className='space-y-1 text-muted-foreground text-sm'>
											...
											{key.expiresAt && <div>过期时间: {format(new Date(key.expiresAt), 'yyyy-MM-dd HH:mm')}</div>}
											{key.lastUsedAt && <div>最后使用: {format(new Date(key.lastUsedAt), 'yyyy-MM-dd HH:mm')}</div>}
										</div>
									</div>
									<div className='flex items-center gap-2'>
										<AlertDialog>
