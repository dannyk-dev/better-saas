...
import { addToast, Badge, Button, Card, CardHeader } from '@heroui/react';
	...
	const fetchCreditBalance = useCallback(async () => {
		try {
			...
			if (result.success && result.data) {
				...
				addToast({
					title: result.error || 'Failed to load credit balance',
					variant: 'flat',
					color: 'danger',
				});
			}
	...
	if (!creditData) {
		return (
			<Card>
				...
				<CardContent>
					<Button onPress={handleRefresh} variant='bordered' size='sm'>
	...
	return (
		<div className='grid gap-4 md:grid-cols-2 lg:grid-cols-4'>
			...
			<Card>
				...
				<CardContent>
					<div className='font-bold text-2xl text-primary'>{creditData.availableBalance.toLocaleString()}</div>
			...
			<Card>
				...
				<CardContent>
					<div className='font-bold text-2xl text-muted-foreground'>{creditData.frozenBalance.toLocaleString()}</div>
			...
			<Card>
				<CardHeader className='flex flex-row items-center justify-between space-y-0 pb-2'>
					...
					<Badge variant='flat' className='h-4 px-1 text-xs'>
						All Time
					</Badge>
				</CardHeader>
				<CardContent>
